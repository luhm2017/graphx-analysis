...
object RunLoadApplyGraphx2 {
  def main(args: Array[String]): Unit = {
    ...
    val degree = 4 //这里是二跳邻居 所以只需要定义为2即可
    ...
    val applyVertexEdge = sc.textFile(applyPath).mapPartitions(lines =>
      lines.map { line =>
        var arr = line.split(",")
        val term_id = if (StringUtils.isNotBlank(arr(7)) && !"null".equals(arr(7).toLowerCase)) arr(7) else "0"
    ...
    var applyVertexRDD: RDD[(Long, String)] = applyVertexEdge.flatMap(k => k._1).filter(k => k._2 != "0").distinct()
    var applyEdgeRdd = applyVertexEdge.flatMap(k => k._2).filter(k => k.srcId != 0L && k.dstId != 0L)

    var callVertexRDD: RDD[(Long, String)] = callVertexEdge.flatMap(k => k._1).filter(k => k._2 != "0").distinct()
    ...
    def sendMsg(e: EdgeTriplet[VMap, _]) = {
      ...
      var srcId = e.srcId.toString
      var dstId = e.dstId.toString

      if (srcId.length > 11 || dstId.length > 11) {
        val arr = analyedID(srcId, dstId, e.srcAttr ++ e.dstAttr)
        srcId = arr(0)
        dstId = arr(1)
      }
    ...
    def analyedID(srcId: String, dstId: String, att: VMap): Array[String] = {
      val totalAttr = att.values.filter(p => StringUtils.isNoneEmpty(p.attr)).map { k =>
    ...
    def addMaps(spmap1: VMap, spmap2: VMap): VMap =
      (spmap1.keySet ++ spmap2.keySet).map {
        k =>
          var result: NDegreeEntity = if (spmap1.get(k).isDefined && spmap2.get(k).isDefined) {
            ...
            spmap11.copy(mergeMsg(spmap11.attr, spmap12.attr), loop)

          } else {

            if (spmap1.get(k).isDefined) spmap1.getOrElse(k, new NDegreeEntity(s"$k->$k", Int.MaxValue))
    ...
    val newG = g.mapVertices((vid, att) => Map[VertexId, NDegreeEntity](vid -> new NDegreeEntity(att, loop = degree)))
